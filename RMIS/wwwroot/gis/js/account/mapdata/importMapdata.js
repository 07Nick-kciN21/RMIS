...
class ImportMapdataApp {
    ...
    handleSubmit(e) {
        ...
        // 檢查每個專案是否已完成照片上傳，收集未完成的專案名稱
        const incompleteProjects = [];
        for (const [projectId, project] of Object.entries(window.projectPhotoData || {})) {
            if ((project.uploadedPhotos || []).length < project.requiredPhotos) {
            incompleteProjects.push(project.name);
            }
        }
        if (incompleteProjects.length > 0) {
            alert(`以下專案的照片尚未上傳完成：\n${incompleteProjects.join('\n')}`);
            return;
        }
