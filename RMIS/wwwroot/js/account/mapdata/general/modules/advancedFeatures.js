...
export class AdvancedFeatures {
    ...
    initAdvancedOptions() {
        ...
        $.ajax({
            url: `/Mapdata/General/Get/LayerConfig?layerId=${id}`,
            type: "GET",
            processData: false,
            contentType: false,
            xhrFields: {
            ...
            success: (data) => {
                if (data.success) {
                    const layerConfig = JSON.parse(data.layerConfig);
                    window.advancedConfig = layerConfig;
                    
                    if (window.advancedConfig.advanced) {
                        if (window.advancedConfig.associated_layer) {
                            window.associatedLayerConfig = window.advancedConfig.associated_layer;
                        }
    ...
    loadAdvancedModules() {
        ...
        // 根據配置載入對應模組
        if (window.advancedConfig.modules && window.advancedConfig.modules.length > 0) {
            window.advancedConfig.modules.forEach(module => {
                switch(module) {
                    case 'photo_upload':
                        ...
                        break;
                    case 'layer_sync':
                        $advancedContainer.append(this.createLayerSyncModule());
                        break;
                    case 'display_settings':
                        $advancedContainer.append(this.createDisplaySettingsModule());
                        break;
                }
            });
            
            // 如果設定自動展開
            if (window.advancedConfig.settings && window.advancedConfig.settings.auto_expand) {
                setTimeout(() => {
                    this.toggleAdvanced();
                }, 100);
            }
